...
void mem_init(size_t size) 
{
    ...
    free_list->size = size;          // Entire pool size
...
void* mem_alloc(size_t size) 
{
    ...
    Block *current_block = free_list, *prev = NULL;
    ...
    while (current_block) 
    {
        ...
        if (current_block->is_free && current_block->size >= size) 
        {
            ...
            if (current_block->size > size) 
            {
                ...
                new_block->size = current_block->size - size;                             // Set size of the new block.
                ...
                current_block->size = size;                                               // Adjust the current block to the requested size.
            ...
            if (prev) prev->next_block = current_block->next_block;
            // Remove the allocated block from the free list
            else free_list = current_block->next_block;

            // Return a pointer to the memory just after the block header (the actual usable memory)
            return (char*)current_block + sizeof(Block);
        }

        // Move to the next block in the list and update the previous pointer
        prev = current_block;
...
void mem_free(void* block) 
{
    ...
    // Link the previous block to the freed block if there is one.
    if (prev) prev->next_block = block_to_free;
    ...
    if (block_to_free->next_block && (char*)block_to_free + sizeof(Block) + block_to_free->size == (char*)block_to_free->next_block) 
    {
        block_to_free->size += sizeof(Block) + block_to_free->next_block->size;              // Merge the two blocks
    ...
    if (prev && (char*)prev + sizeof(Block) + prev->size == (char*)block_to_free) 
    {
        prev->size += sizeof(Block) + block_to_free->size;              // Merge the two blocks
        prev->next_block = block_to_free->next_block;                   // Update the next pointer
    }
}
...
void* mem_resize(void* block, size_t size) 
{
    ...
    if (old_block->size >= size) return block;
    ...
    if (new_block) 
    {
        memcpy(new_block, block, old_block->size);  // Copy old data to new block
